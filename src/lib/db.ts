...
export async function createDailyScore(
  userId: string,
  roomId: string,
  scoreDate: Date,
  score: number
  ...
  const result = await query(
    'INSERT INTO daily_scores (user_id, room_id, score_date, score) VALUES ($1, $2, $3, $4) RETURNING *',
...
export async function getDailyScoresByRoom(roomId: string): Promise<DailyScore[]> {
  const result = await query(
    'SELECT ds.*, u.name, u.username FROM daily_scores ds JOIN users u ON ds.user_id = u.id WHERE ds.room_id = $1 ORDER BY ds.score_date ASC',
    [roomId]
  )
  return result.rows
}

export async function getDailyScoresByUser(userId: string, roomId: string): Promise<DailyScore[]> {
  const result = await query(
    'SELECT * FROM daily_scores WHERE user_id = $1 AND room_id = $2 ORDER BY score_date ASC',
    [userId, roomId]
  )
  return result.rows
}
...
export async function initializeDatabase(): Promise<void> {
  const createTables = `
    ...
    -- Create daily_scores table for graph visualization
